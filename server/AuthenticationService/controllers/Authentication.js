...
class AuthenticationController {
    async register(req, res) {
        const user = await AuthenticationService.register(req.body.name, req.body.email, req.body.password);
        res
    ...
    async verifyMfa(req, res) {
        ...
        const result = await AuthenticationService.verifyMfa(email, token, tempMfaToken);
        res.status(StatusCodes.OK).json(result);
    }
    ...
    async resetPassword(req, res) {
        ...
        const result = await AuthenticationService.resetPassword(resetToken, password);
        res.status(StatusCodes.OK).json(result);
    }
}
